{ inputs, ... }:
{
  perSystem =
    ...
    let
      ...
      # kitty only recognises `+kitten` as its first argument, so the config
      # must not be injected via `--config`: that would turn `kitty +kitten
      # icat` (nnn's image preview) into a terminal window trying to exec a
    ...
    in
    {
      packages.kitty = inputs.wrappers.lib.wrapPackage {
        inherit pkgs;
        package = pkgs.kitty;
        env.KITTY_CONFIG_DIRECTORY = "${configDir}";
